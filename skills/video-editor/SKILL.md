...
- ffmpeg recipe: supersampled animated crop, never `zoompan` (`zoompan` snaps coordinates to whole pixels, so slow moves step visibly). `scale=3840:2160:flags=lanczos`, then `crop` with per-frame `w/h/x/y` from `n` (frame number, 30 fps), then `scale=1920:1080:flags=lanczos`. Rounding errors land sub-pixel after the downscale, so the move glides.
- Ramp `up = smoothstep((n-60)/60)` for t=2-4 s, `down = smoothstep((n-420)/60)` for t=14-16 s, `k = up*(1-down)`, `z = 1+0.4118*k`. Center lerps frame-center to punch-center by `k` plus drift (`cx = 1920+360*k+70*d2`, `cy = 1080+285*k+24*d2`). Window `w = 3840/z`, `h = 2160/z`, clamped inside the frame. Write the filter with `-filter_complex_script`, never inline quoting. This build's `crop` has no `eval` switch and rejects zoompan's `on` variable (init-time NaN aborts the encode); use `n`.
